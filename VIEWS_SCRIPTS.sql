...
SELECT SEASON, MEAL_TYPE, COUNT(MEAL_ID) AS BOOKING_COUNT FROM (SELECT
    CASE
    ...
    END AS SEASON, M.TYPE AS MEAL_TYPE, B.MEAL_ID
FROM BOOKING B
LEFT JOIN MEAL M ON B.MEAL_ID = M.MEAL_ID)
GROUP BY SEASON, MEAL_TYPE)                       --TO_CHAR(B.DATE_OF_DELIVERY, 'MM')
   SELECT SEASON, MEAL_TYPE, BOOKING_COUNT,Ranking from (Select SEASON, MEAL_TYPE, BOOKING_COUNT, 
